...
This scraper extracts job listings from FREQUENTIS careers page (Romania only) and imports them to peviitor.ro.

Target: https://jobs.frequentis.com/careers/SearchJobs
...
- **Cheerio** - For HTML parsing (FREQUENTIS uses Avature ATS)
- **OpenCode + Big Pickle** - For development

## Workflow Steps

1. **Start with brand** - We know the brand (e.g., "FREQUENTIS")
...
8. **Scrape new jobs** - Extract jobs from FREQUENTIS careers page (Romania)
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts FREQUENTIS Careers jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from FREQUENTIS careers HTML (Romania only)
...
|----------|-------------|
| `SOLR_AUTH` | SOLR credentials in format `user:password` (default: `solr:SolrRocks`) |
