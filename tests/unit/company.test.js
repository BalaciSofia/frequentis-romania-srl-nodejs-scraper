...
describe('company.js', () => {
  ...
  describe('getCompanyData (no cache)', () => {
    it('should find FREQUENTIS via ANAF search and return company data', async () => {
      ...
      expect(result).toHaveProperty('company', 'FREQUENTIS ROMANIA SRL');
      expect(result).toHaveProperty('cif', '25475641');
      expect(result).toHaveProperty('active', true);
      expect(result).toHaveProperty('anafData');
      expect(result.anafData.name).toBe('FREQUENTIS ROMANIA SRL');
    });
    ...
    it('should throw when no active company found', async () => {
      mockFetch.mockResolvedValueOnce(anafSearchResponse([
        { cui: 11111111, name: 'FREQUENTIS SOMETHING SRL', statusLabel: 'Radiata' }
      ]));
  ...
  describe('validateAndGetCompany', () => {
    it('should return company data with status active', async () => {
      mockFetch
        ...
        .mockResolvedValueOnce(peviitorResponse([{ company: 'FREQUENTIS ROMANIA SRL' }]));
    ...
    it('should return inactive status when company is inactive', async () => {
      const inactiveRecord = { ...FREQ_ANAF_RECORD, inactive: true };
